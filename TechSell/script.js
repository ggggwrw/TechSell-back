...
// Usa el perfil dev (H2) en puerto 8081 mientras configuras MySQL
...
if (contenedor) {
  try {
    ...
    const productosManual = (globalThis.productosManual || []);
    const productos = [...mapped, ...productosManual];
    ...
    console.error('Fallo backend, mostrando productos manuales:', err);
    const productosManual = (globalThis.productosManual || []);
    for (const producto of productosManual) {
      const card = document.createElement('div');
      card.className = 'producto';
      card.innerHTML = `
        <div class="producto-img-container">
...
async function agregarAlCarrito(id) {
  const productoManual = (globalThis.productosManual || []).find(p => p.id === id);
  if (productoManual) {
...
function cambiarCantidad(index, cantidad) {
  carrito[index].cantidad = Number.parseInt(cantidad, 10);
  actualizarContadorCarrito();
  mostrarCarrito();
}
...
if (globalThis.location?.pathname.includes('carrito.html')) {
  mostrarCarrito();
}
